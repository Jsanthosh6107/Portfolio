...
const TilesProjects = ({tilesHeader = "", tilesTitle = "", tiles = []}) => {
  ...
  return (
    <section className="x-margin my-40" id="activeworks">
          ...
          <div className="flex justify-center flex-wrap gap-6">
              {tiles.map((tile, index) => (
                  <SlideIn
                      key={index}
                      delay={(index * 100) + 200}
                      classes="w-[90%] md:w-[45%] lg:w-[30%] relative rounded-[25px] flex flex-col"
                  >
                      ...
                      <div className="px-8 py-6 border-gray-400 border-x-2 border-b-2 rounded-b-[25px] bg-white shadow-2xl flex flex-col flex-grow">
                          <h3 className="font-medium text-2xl">{tile.title}</h3>
                          <p className="flex-grow">{tile.description}</p>
                      </div>
                  </SlideIn>
              ))}
          </div>
